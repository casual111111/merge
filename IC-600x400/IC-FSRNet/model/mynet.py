...
class LFSRNet(nn.Module):
    ...
    #####///////////256x256///////////////////////////////////////////////////
        ...
        grid = grid.view(grid.shape[0], 64, 4, int(self.args.input_size_x/8),  int(self.args.input_size_y/8))#grid[1, 64, 4, 50, 75]
        ...
        grid = grid.view(grid.shape[0], 64, int(self.args.input_size_x/4), int(self.args.input_size_y/4))#(1,64,100,150))
        grid = self.refine_grid[0](grid, x3) #torch.Size([1, 64, 16, 16])#Illumination Refinement#(64,624,624)


        ###100, 150
        grid = grid.view(grid.shape[0], 64, 4, int(self.args.input_size_x/8),  int(self.args.input_size_y/8))#(64,4,312,312)
        inp3 = self.down_stage2(x3, guidance[2], grid) ####(64,624,624)
        inp2 = inp3+ x3
        x2= self.up21(inp2)###(64,1248,1248)
        grid = grid.view(grid.shape[0],64,  int(self.args.input_size_x/4), int(self.args.input_size_y/4))
        grid = self.refine_grid[1](grid, x2)
        

        #//1248
        grid = grid.view(grid.shape[0], 64, 4, int(self.args.input_size_x/8),  int(self.args.input_size_y/8))
        inp2= self.up2_stage1(x2, guidance[1], grid)###(64,1248,1248)
        inp4 = inp2 + x4
        x1= self.up22(inp4)#(1,64,2496,2496)
        grid = grid.view(grid.shape[0],64,  int(self.args.input_size_x/4), int(self.args.input_size_y/4))
