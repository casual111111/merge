...
def hiseq_color_cv2_img_rgb(img):
    (b, g, r) = cv2.split(img)
...
def hiseq_color_cv2_img(img):
    ...
    equalized_hsv_img = cv2.merge((h, s, vH))
    return equalized_hsv_img
...
def stretch_linear(betas):
    ...
    while ans_r - ans_l >= 1e-7:
        ans_m = (ans_l + ans_r) / 2
        if check_end_beta(beta0=betas[0], mid_beta=mid_beta, end_beta=ans_m, n=n,
                          alphas_cumprod_limit=alphas_cumprod[-1]):
            end_beta = ans_m
            ans_r = ans_m
        else:
            ans_l = ans_m
