...
import torch
from icnet import common,DropPath
...
class OutputProj(nn.Module):
    ...
    def forward(self, x, H, W):
        B, L, C = x.shape
        assert H * W == L, f"Token length {L} != H*W ({H}*{W})"
...
class SingleAttention(nn.Module):#空间注意力
    ...
    def forward(self, rgb_fea, depth_fea):
        ...
        rgb_fea = (rgb_attn @ depth_v).transpose(1, 2).reshape(B, N, C)#加权求和
...
class SingleFusionTransformer(nn.Module):
    ...
    def forward(self, rgb_fea, depth_fea):
        ...
        rgb_fea_fuse = self.drop_path(
            self.singleAttn(rgb_fea, depth_fea))#提取K，Q，V，并做自注意力

        rgb_fea = rgb_fea + rgb_fea_fuse
        # depth_fea = depth_fea + depth_fea_fuse

        rgb_fea = rgb_fea + self.drop_path(self.mlp_rgb_ma(self.norm3_rgb_ma(rgb_fea)))#全连接层和drop
...
class refineblock4(nn.Module):
    ...
    def forward(self, x, save, H, W):
        save = F.interpolate(save, size=(H, W), mode='bicubic', align_corners=False)
        save = self.input_proj(save)
        x = self.input_proj(x)
        assert x.shape[1] == save.shape[1], f"Token mismatch: {x.shape[1]} vs {save.shape[1]} (expected {H*W})"
        x1=self.block(x, save)##x(1,4096,8) (1,256,8)
        x = self.output_proj(x1, H, W)
        x = self.enhance(x)
        return x
