...
class BiaGroup(nn.Module):
    ...
    def forward(self, x, guidance, grid):
        res = x
        for i in range(len(self.body)):#6次循环
...
class BasicBlock2(nn.Module):
    ...
    def forward(self, x, guide, coeffs):
        x = self.basic1(x)#（1, 64, 200, 300）#Super-Resolution RCAB
        _, c, h, w = x.shape
        res = self.conv2(self.relu(self.conv1(x)))
        res = F.tanh(res) #S1（1，64，200，300）
        slice_coeffs = self.slice(coeffs, guide) #3D Slicing（64，1248，1248）
        slice_coeffs = self.adjust(slice_coeffs) #c1->Illumination Adjustment上分支（64，1248，1248）
        out = self.apply_coeffs(slice_coeffs, res) #IATF

        out = out + x

        return out#（64，1248，1248）
